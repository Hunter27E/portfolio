...
function removeListeners(modalName) {
	let selector1 = `#${modalName} i:first-of-type`;
	let selector2 = `#${modalName} i:last-child`;
...
const addBlog = () => {
	...
	document.querySelector('#disabler').style.display = 'flex';
	// add event listeners to cancel/add buttons
	document
		.querySelector('#addModal i:first-of-type')
		.addEventListener('click', closeAddModal, true);
	document
		.querySelector('#addModal i:last-child')
...
const updateBlog = (e) => {
	...
	document
		.querySelector('#updateModal i:first-of-type')
		.addEventListener('click', closeUpdateModal, true);
	document
		.querySelector('#updateModal i:last-child')
...
const finishDelete = () => {
	...
	// delete the section containing this blog from DOM
	//blog.parentElement.removeChild(blog); no longer needed
	blog.parentElement.parentElement.removeChild(blog.parentElement);
...
const deleteBlog = (e) => {
	...
	document
		.querySelector('#deleteModal i:first-of-type')
		.addEventListener('click', closeDeleteModal, true);
	document
		.querySelector('#deleteModal i:last-child')
...
function createBlog(blog) {
	...
	updateIcon.setAttribute('class', 'material-icons');
	updateIcon.textContent = 'edit';
	updateIcon.addEventListener('click', updateBlog, true);
	let delIcon = document.createElement('i');
	delIcon.setAttribute('class', 'material-icons');
	delIcon.textContent = 'delete';
	delIcon.addEventListener('click', deleteBlog, true);
